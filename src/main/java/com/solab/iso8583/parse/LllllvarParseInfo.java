...
public class LllllvarParseInfo extends FieldParseInfo {
	...
	throws ParseException, UnsupportedEncodingException {
		...
		if (_v.length() != len) {
			_v = new String(buf, pos + 5, buf.length-pos-5, getCharacterEncoding()).substring(0, len);
		}
		if (custom == null) {
	...
	public <T> IsoValue<?> parseBinary(final int field, final byte[] buf,
		...
		final int len = ((buf[pos] & 0x0f) * 10000) + (((buf[pos + 1] & 0xf0) >> 4) * 1000) + ((buf[pos + 1] & 0x0f) * 100) + (((buf[pos + 2] & 0xf0) >> 4) * 10) + (buf[pos + 2] & 0x0f);
